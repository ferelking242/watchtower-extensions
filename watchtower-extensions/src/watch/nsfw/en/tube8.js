const watchtowerSources = [{
    ...
    "itemType": 1,
    "version": "1.1.0",
    "pkgPath": "watch/nsfw/en/tube8.js",
    "notes": "Adult content (18+) — ZeusDL powered streaming",
    "isNsfw": true
  }];
  class DefaultExtension extends MProvider {
    getHeaders(url) {
      return { "Referer": "https://www.tube8.com/", "User-Agent": "Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36", "Cookie": "age_verified=1" };
    }
    async getPopular(page) {
      const url = `https://www.tube8.com/${page > 1 ? "?page=" + page : ""}`;
      const res = await new Client().get(url, this.getHeaders(url));
      return this._parse(res.body);
    }
    get supportsLatest() { return true; }
    async getLatestUpdates(page) {
      const url = `https://www.tube8.com/newest/${page > 1 ? "?page=" + page : ""}`;
      const res = await new Client().get(url, this.getHeaders(url));
      return this._parse(res.body);
    }
    async search(query, page, filters) {
      const q = encodeURIComponent(query.trim());
      const url = `https://www.tube8.com/search.html?q=${q}&page=${page}`;
      const res = await new Client().get(url, this.getHeaders(url));
      return this._parse(res.body);
    }
    _parse(html) {
      ...
      for (const card of cards) {
        ...
        items.push({ name: title.trim(), imageUrl: thumb, link: href.startsWith("http") ? href : "https://www.tube8.com" + href, description: dur ? `Duration: ${dur}` : "" });
      }
    ...
    async getDetail(url) {
      ...
      const tags = doc.select(".video_tags a, .tag").map(el => ({ name: el.text.trim() }));
