const watchtowerSources = [{
  ...
  "apiUrl": "",
  "iconUrl": "https://www.freepornvideos.xxx/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.1.0",
  "pkgPath": "watch/nsfw/en/freepornvideos.js",
...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    ...
    const url = `https://www.freepornvideos.xxx/${from > 0 ? `?from=${from}` : ""}`;
    const res = await new Client().get(url, this.getHeaders(url));
    return this._parseList(res.body, "https://www.freepornvideos.xxx");
  }
  get supportsLatest() { return true; }
  async getLatestUpdates(page) {
    const from = (page - 1) * 60;
    const url = `https://www.freepornvideos.xxx/new/${from > 0 ? `?from=${from}` : ""}`;
    const res = await new Client().get(url, this.getHeaders(url));
    return this._parseList(res.body, "https://www.freepornvideos.xxx");
  }
  async search(query, page, filters) {
    const q = encodeURIComponent(query.trim());
    const url = `https://www.freepornvideos.xxx/search/?q=${q}`;
    const res = await new Client().get(url, this.getHeaders(url));
    return this._parseList(res.body, "https://www.freepornvideos.xxx");
  }
