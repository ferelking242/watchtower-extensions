  ...
  class MangaReader extends MProvider {
    ...
    Future<MManga> getDetail(String url) async {
      ...
      for (var element in elements) {
        ...
        final chUrl = urlElements?.attr("href") ?? "";
      ...
      if (pages.isEmpty || pages.length == 1) {
        try {
          ...
          if (images.isNotEmpty) {
            final decoded = json.decode(images) as List;
            for (var page in decoded) {
              if (page != null) pagesUrl.add(page.toString());
            }
          }
        } catch (e) {}
      } else {
    ...
    MPages mangaRes(String res) {
      ...
      for (var element in elements) {
        ...
        var manga = MManga();
        manga.name = element.selectFirst("a")?.attr("title") ?? element.selectFirst("a")?.text ?? "";
        manga.imageUrl = img;
    ...
    String getMangaUrlDirectory(String sourceName) {
      if (["Sushi-Scan", "Sushi-Scans"].contains(sourceName)) {
        return "/catalogue";
      }
      return "/manga";
    }
    ...
    String getMangaSearchUrl(int page, String query) {
      if (["Sushi-Scan", "Sushi-Scans"].contains(source.name)) {
    ...
    bool ignoreFilter() {
      return ["Sushi-Scan", "Sushi-Scans"].contains(source.name);
    }
  }
