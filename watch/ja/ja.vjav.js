...
class DefaultExtension extends MProvider {
  ...
  async getPopular(page) {
    const url = `${this.source.baseUrl}/videos/filtered/?t=mr&m=${page}`;
  ...
  async getLatestUpdates(page) {
    const url = `${this.source.baseUrl}/videos/filtered/?t=n&m=${page}`;
    const res = await new Client().get(url, { headers: this.getHeaders(url) });
    return this._parse(res.body, this.source.baseUrl);
  }
  ...
  async search(query, page, filters) {
    ...
    const url = `${this.source.baseUrl}/videos/filtered/?q=${q}&m=${page}`;
    const res = await new Client().get(url, { headers: this.getHeaders(url) });
    return this._parse(res.body, this.source.baseUrl);
  }
