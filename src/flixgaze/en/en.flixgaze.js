     ...
     class DefaultExtension extends MProvider {
       ...
       _parseList(html, page) {
         ...
         if (cards && cards.length > 0) {
           for (const card of cards) {
             ...
             if (!thumb) continue; // require thumbnail — nav items never have one
       ...
       async getDetail(url) {
         ...
         for (const sel of [".cat-links a", ".genre a", "a[href*='/genre/']"]) {
           ...
           if (els && els.length > 0) {
             for (const el of els) { const t = (el.text || "").trim(); if (t && t.length > 1 && tags.findIndex(x => x.name === t) < 0) tags.push({ name: t }); }
       ...
       async getVideoList(url) {
         ...
         if (zm) { const u = zm[2] + "/" + zm[1] + "/" + zm[3] + ".m3u8"; return [{ url: u, quality: "HLS · ZeusDL", originalUrl: u }]; }
         const videos = [], seen = [];
         const re = /["'](https?:\/\/[^"']+\.(?:m3u8|mp4)[^"']*?)["']/g;
         let m;
         while ((m = re.exec(html)) !== null) { if (!seen.includes(m[1])) { seen.push(m[1]); videos.push({ url: m[1], quality: m[1].includes("m3u8") ? "HLS" : "MP4", originalUrl: m[1] }); } }
         if (videos.length === 0) {
           const doc2 = new Document(html);
           const iframes = doc2.select("iframe[src], iframe[data-src]");
           if (iframes) for (const f of iframes) { const src = f.attr("src") || f.attr("data-src") || ""; if (src && !seen.includes(src)) { seen.push(src); videos.push({ url: src, quality: "Embed", originalUrl: src }); } }
         }
       ...
       getFilterList() {
         return [{
           type_filter: "SelectFilter", name: "Type", state: 0,
           values: [
             { name: "All", value: "" }, { name: "Movies", value: "movie" },
             { name: "TV Series", value: "tv-series" }, { name: "Foreign Movies", value: "foreign-movies" },
             { name: "Marvel Cinematic Universe", value: "marvel-cinematic-universe" }
           ]
         }, { type_filter: "TextFilter", name: "Year", state: "" }];
       }
