const watchtowerSources = [{
    "name": "LyricalNonsense",
    "lang": "en",
    "baseUrl": "https://lyrical-nonsense.com",
    "apiUrl": "https://lyrical-nonsense.com",
    "iconUrl": "https://lyrical-nonsense.com/favicon.ico",
    "typeSource": "single",
    "itemType": 2,
    "version": "0.1.0",
    "pkgPath": "music/en/lyricalnonsense.js",
    "notes": "LyricalNonsense — anime song lyrics & translations",
    "editableBaseUrl": true
}];

const BASE_URL = "https://lyrical-nonsense.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/global/top-lyrics?page=${page}`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/global/latest-lyrics?page=${page}`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async search(query, page, filters) {
        if (!query) return this.getPopular(page);
        const res = await new Client().get(`${this.getBaseUrl()}/global/search?keyword=${encodeURIComponent(query)}&page=${page}`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getDetail(url) {
        ...
        const descM = html.match(/<div[^>]*class="[^"]*lyrics-body[^"]*"[^>]*>([\s\S]*?)<\/div>/i) || html.match(/<div[^>]*class="[^"]*romaji[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
