const watchtowerSources = [{
    ...
    "lang": "en",
    "baseUrl": "https://animesonglyrics.com",
    "apiUrl": "https://animesonglyrics.com",
    "iconUrl": "https://animesonglyrics.com/favicon.ico",
    "typeSource": "single",
    "itemType": 2,
    "version": "0.1.0",
    "pkgPath": "music/en/animesonglyrics.js",
    "notes": "AnimeSongLyrics — anime song lyrics database",
    "editableBaseUrl": true
}];

const BASE_URL = "https://animesonglyrics.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/lyrics?page=${page}`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/lyrics?page=${page}&sort=newest`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    ...
    async getDetail(url) {
        ...
        const descM = html.match(/<div[^>]*class="[^"]*lyrics[^"]*"[^>]*>([\s\S]*?)<\/div>/i) || html.match(/<pre[^>]*>([\s\S]*?)<\/pre>/i);
