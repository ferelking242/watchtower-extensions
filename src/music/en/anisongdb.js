const watchtowerSources = [{
    "name": "AnisongDB",
    "lang": "en",
    "baseUrl": "https://anisongdb.com",
    "apiUrl": "https://anisongdb.com",
    "iconUrl": "https://anisongdb.com/favicon.ico",
    "typeSource": "single",
    "itemType": 2,
    "version": "0.1.0",
    "pkgPath": "music/en/anisongdb.js",
    "notes": "AnisongDB — anime song database with OP/ED info",
    "editableBaseUrl": true
}];

const BASE_URL = "https://anisongdb.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/all?page=${page}`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/all?page=${page}&sort=newest`, this.getHeaders());
        return { list: this.parseSongList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    ...
    async getDetail(url) {
        ...
        const descM = html.match(/<div[^>]*class="[^"]*info[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
