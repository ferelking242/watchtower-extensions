...
class DefaultExtension extends MProvider {
    ...
    async getPopular(p) { const r=await new Client().get(`${BASE_URL}/anime/?order=popular&page=${p}`,this.getHeaders());return{list:this.parseList(r.body),hasNextPage:r.body.includes('page=')}; }
    async getLatestUpdates(p) { const r=await new Client().get(`${BASE_URL}/anime/?order=update&page=${p}`,this.getHeaders());return{list:this.parseList(r.body),hasNextPage:r.body.includes('page=')}; }
    async search(q,p) { if(!q)return this.getPopular(p);const r=await new Client().get(`${BASE_URL}/?s=${encodeURIComponent(q)}`,this.getHeaders());return{list:this.parseList(r.body),hasNextPage:false}; }
