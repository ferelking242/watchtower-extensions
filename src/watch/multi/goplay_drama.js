const watchtowerSources = [{
    "name": "GoPlayDrama",
    "lang": "multi",
    "baseUrl": "https://goplay.to",
    "apiUrl": "https://goplay.to",
    "iconUrl": "https://goplay.to/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/multi/goplay_drama.js",
    "notes": "GoPlayDrama — drama & Asian content streaming",
    "editableBaseUrl": true
}];

const BASE_URL = "https://goplay.to";
...
class DefaultExtension extends MProvider {
    ...
    async getDetail(url) {
        ...
        const episodes = [];
        const epRe = /<a[^>]+href="(\/[^"]*(?:episode|ep)[^"]*)"[^>]*>\s*([^<]*(?:Episode|Ep\.?)\s*\d+[^<]*)<\/a>/gi;
        let em;
        while ((em = epRe.exec(html)) !== null) { episodes.push({ name: em[2].trim(), url: em[1], dateUpload: "" }); }
        if (episodes.length === 0) {
            const epRe2 = /href="(\/[^"]*ep[^"]*)"[^>]*>\s*(\d+)/gi;
