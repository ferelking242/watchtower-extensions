const watchtowerSources = [{
    ...
    "isNsfw": false,
    "hasCloudflare": false,
    "pkgPath": "watch/src/multi/moviebox-series.js"
}];

// ═══════════════════════════════════════════════════════
//  MovieBox Series — aoneroom.com API v2 + v3
//  subjectType=2 (TV Series)
...
class DefaultExtension extends MProvider {
    ...
    static get SUBJECT_TYPE() { return 2; }
    ...
    async getPopular(page) { return this.postSearch("", page, DefaultExtension.SUBJECT_TYPE, "VIEWS"); }
    async getLatestUpdates(page) { return this.postSearch("", page, DefaultExtension.SUBJECT_TYPE, "NEW"); }
    async search(query, page, filterList) { return this.postSearch(query, page, DefaultExtension.SUBJECT_TYPE, null); }
    ...
    buildEpisodes(subjectId, detailPath, seasonInfo) {
        ...
        if (seasonInfo && seasonInfo.seasons && Array.isArray(seasonInfo.seasons)) {
            for (const season of seasonInfo.seasons) {
                ...
                if (epList.length > 0) {
                    ...
                    // Season exists but no episode list — use episode count
                    const epCount = season.episodeCount || season.totalEpisode || season.episodeNum || 12;
        ...
        return [
            {
                ...
                "listPreference": {
                    ...
                    "valueIndex": 0,
                    "entries": ["English", "Français", "العربية", "Português", "Indonesian", "中文", "Русский", "Filipino", "Kiswahili", "বাংলা", "اُردُو"],
                    "entryValues": ["en", "fr", "ar", "pt", "id", "zh", "ru", "tl", "sw", "bn", "ur"]
                }
            }
        ];
    }
