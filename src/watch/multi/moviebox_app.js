...
// ══════════════════════════════════════════════════════════════
//  MovieBox App  v1.0.0
//  API native app découverte par reverse-engineering APK v3.0.15 :
//   - _fetchPlay() : wefeed-mobile-bff/subject-api/play-info
//     (api3.aoneroom.com) → retourne tous les streams HLS + MP4
//     multi-qualités + multi-langues.
//   - Fallback H5 si l'API native retourne 403 (région bloquée).
//   - Captions : wefeed-mobile-bff/subject-api/get-stream-captions
    ...
    async getVideoList(url) {
        ...
        try {
            ...
            if (stream && stream.id) {
                ...
                // Essai captions API native d'abord, puis H5 fallback
                var cRes = null;
                try {
                    var capMUrl = MB_MOBILE_API + "/wefeed-mobile-bff/subject-api/get-stream-captions"
                        ...
                        + "&subjectId=" + encodeURIComponent(subjectId)
                        + "&detailPath=" + encodeURIComponent(detailPath);
                    var cMRes = await new Client().get(capMUrl, mbHeaders(lang, detailPath));
                ...
                if (!cRes) {
                    var capUrl = MB_API + "/wefeed-h5api-bff/subject/caption"
                        ...
                        + "&subjectId=" + encodeURIComponent(subjectId)
                        + "&detailPath=" + encodeURIComponent(detailPath);
                    cRes = await new Client().get(capUrl, mbHeaders(lang, detailPath));
                }
        ...
        function _buildVideoLabel(item, fmt, idx) {
            ...
            // Normalise la résolution — API native utilise "definition" ou "clarity"
            // ex: "HD" → 720p, "FHD" → 1080p, "4K" → 4K, "SD" → 480p, "LD" → 360p
            var DEF = {
                "4k": "4K", "uhd": "4K", "2k": "2160p",
                "fhd": "1080p", "hq": "1080p", "od": "1080p",
                "hd": "720p",
                "sd": "480p",
                "ld": "360p", "sq": "360p", "fd": "360p",
                "auto": ""
            };
