const watchtowerSources = [{
    "name": "Einthusan",
    "lang": "multi",
    "baseUrl": "https://einthusan.com",
    "apiUrl": "https://einthusan.com",
    "iconUrl": "https://einthusan.com/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/multi/einthusan.js",
    "notes": "Einthusan — Indian/regional movie streaming",
    "editableBaseUrl": true
}];

const BASE_URL = "https://einthusan.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/hindi/page/${page}`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page/${page + 1}`) };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/hindi/recent/page/${page}`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page/${page + 1}`) };
    }
    async search(query, page, filters) {
        if (!query) return this.getPopular(page);
        const res = await new Client().get(`${this.getBaseUrl()}/movie/${encodeURIComponent(query)}`, this.getHeaders());
