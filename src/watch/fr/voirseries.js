const watchtowerSources = [{
      ...
      "langs": ["fr"],
      "ids": { "fr": 538291740 },
      "baseUrl": "https://voirseries.io",
      "apiUrl": "https://voirseries.io",
      "iconUrl": "https://voirseries.io/favicon.ico",
      "typeSource": "single",
      "itemType": 1,
      "version": "0.2.3",
      "pkgPath": "watch/fr/voirseries.js",
  ...
  const BASE_URL = "https://voirseries.io";
  class DefaultExtension extends MProvider {
      ...
      _parse(html){
          const list=[],seen={};
          const re=/<a[^>]+href="([^"]+)"[^>]*>[\s\S]{0,400}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,100})"/gi;
          let m;
          while((m=re.exec(html))!==null){
              const url=m[1].startsWith("http")?m[1]:this.baseUrl+(m[1].startsWith("/")?m[1]:"/"+m[1]);
              if(url in seen||url===this.baseUrl)continue;
              seen[url]=1;list.push({link:url,imageUrl:m[2],name:this._decode(m[3].trim())});
          }
          return list;
      }
      async getPopular(page){const r=await new Client().get(this.baseUrl+"/series-vf-vostfr/page/"+page+"/",this._hdrs());return{list:this._parse(r.body),hasNextPage:page<20};}
      ...
      async getDetail(url){
          ...
          const eps=[],eSeen={};
          const eRe=/<a[^>]+href="([^"]+(?:episode|saison|ep-|s[0-9])[^"]*)"[^>]*>([\s\S]{1,80}?)<\/a>/gi;let em;
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://voirseries.io",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://voirseries.io"
                }
            },
            {
                key: "default_quality",
