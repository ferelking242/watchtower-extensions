// AnimesUltra — extension Watchtower v1.0.0
// Anime VF/VOSTFR — animesultra.fr

const watchtowerSources = [{
    "name": "AnimesUltra",
    "langs": ["fr"],
    "ids": { "fr": 715021475 },
    "baseUrl": "https://animesultra.fr",
    "apiUrl": "https://animesultra.fr",
    "iconUrl": "https://animesultra.fr/favicon.ico",
    "typeSource": "single",
    "itemType": 2,
    "version": "1.1.1",
    "pkgPath": "watch/fr/animesultra.js",
    ...
    "isAggregator": false,
    "paywall": "free",
    "notes": "AnimesUltra — animesultra.fr — VF/VOSTFR."
}];

const BASE_URL = "https://animesultra.fr";
...
class DefaultExtension extends MProvider {
    ...
    _parseCards(html) {
        ...
        const re = /<a[^>]+href="((?:https?:\/\/[^"]+)?\/(?:anime|serie|film|catalogue)[^"]+)"[^>]*>[\s\S]{0,500}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,100})"/gi;
        let m;
    ...
    async getPopular(page) {
        const url = page <= 1 ? this.baseUrl + "/catalogue/" : `${this.baseUrl}/catalogue/page/${page}/`;
    ...
    async getDetail(url) {
        ...
        const chapters = [];
        const epRe = /<a[^>]+href="([^"]*(?:episode|saison|ep|vf|vostfr)[^"]*)"[^>]*>([\s\S]{0,80}?)<\/a>/gi;
        let em; const eSeen = {};
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://animesultra.fr",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://animesultra.fr"
                }
            },
            {
                key: "default_quality",
