// MavAnimes — extension Watchtower v1.0.0
// Anime VF — mavanimes.co (alternative/miroir)

const watchtowerSources = [{
    "name": "MavAnimes (Alt)",
    "langs": ["fr"],
    "ids": { "fr": 735841695 },
    "baseUrl": "https://mavanimes.cc",
    "apiUrl": "https://mavanimes.cc",
    "iconUrl": "https://mavanimes.cc/favicon.ico",
    "typeSource": "single",
    "itemType": 2,
    "version": "1.1.1",
    "pkgPath": "watch/fr/mavanimes2.js",
    "editableBaseUrl": true,
    "hasCloudflare": false,
    "customUserAgent": "",
    "videoQualities": ["AUTO", "VF"],
    "subCategories": ["anime"],
    "supportsForYou": true,
    ...
    "isAggregator": false,
    "paywall": "free",
    "notes": "MavAnimes Alt — mavanimes.cc — Anime VF."
}];

const BASE_URL = "https://mavanimes.cc";
...
class DefaultExtension extends MProvider {
    ...
    _parseCards(html) {
        ...
        const re = /<a[^>]+href="((?:https?:\/\/[^"]+)?\/(?:anime|serie)[^"]+)"[^>]*>[\s\S]{0,500}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,100})"/gi;
        let m;
    ...
    async getPopular(page) {
        const url = page <= 1 ? this.baseUrl + "/animes-vf/" : `${this.baseUrl}/animes-vf/page/${page}/`;
    ...
    async getDetail(url) {
        ...
        const chapters = [];
        const epRe = /<a[^>]+href="([^"]*(?:episode|saison|ep|vf)[^"]*)"[^>]*>([\s\S]{0,80}?)<\/a>/gi;
        let em; const eSeen = {};
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://mavanimes.cc",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://mavanimes.cc"
                }
            },
            {
                key: "default_quality",
