...
const BASE_URL = "https://echodumarteau.fr";
...
class DefaultExtension extends MProvider {
    ...
    get logTopic() { const p = new SharedPreferences().get("log_topic"); return (p && p.value) ? p.value : "wtfr-echodumarteau"; }
    ...
    async _log(msg) {
        ...
        try { await new Client().post(`https://ntfy.sh/${this.logTopic}`, `[EdM] ${msg}`, { "Title": "EchoDuMarteau", "Content-Type": "text/plain" }); } catch(e) {}
    }
    ...
    async search(query, page, filterList) {
        ...
        const res = await new Client().get(`${this.baseUrl}/?s=${encodeURIComponent(query)}&page=${page}`, this._hdrs());
        const list = this._parse(res.body);
