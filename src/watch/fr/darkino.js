const watchtowerSources = [{
      ...
      "langs": ["fr"],
      "ids": { "fr": 318475029 },
      "baseUrl": "https://darkino.one",
      "apiUrl": "https://darkino.one",
  ...
  class DefaultExtension extends MProvider {
      ...
      _parse(html){
          const list=[],seen={};
          const re=/<article[^>]*>[\s\S]{0,100}?<a[^>]+href="([^"]+)"[^>]*>[\s\S]{0,300}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,120})"/gi;
          let m;
          while((m=re.exec(html))!==null){const url=m[1].startsWith("http")?m[1]:this.baseUrl+m[1];if(url in seen)continue;seen[url]=1;list.push({link:url,imageUrl:m[2],name:this._decode(m[3].trim())});}
          return list;
      }
      async getPopular(page){const r=await new Client().get(this.baseUrl+"/films/page/"+page+"/",this._hdrs());return{list:this._parse(r.body),hasNextPage:page<15};}
      ...
      async getDetail(url){
          ...
          const eps=[],eSeen={};
          const eRe=/<a[^>]+href="([^"]+(?:episode|saison|ep-)[^"]*)"[^>]*>([\s\S]{1,80}?)<\/a>/gi;let em;
      ...
      async getVideoList(url){
          ...
          const vRe=/["']file["']\s*:\s*["']([^"']+\.(?:m3u8|mp4)[^"']*)["']/gi;
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://darkino.one",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://darkino.one"
                }
            },
            {
                key: "default_quality",
