// Nova-Stream — extension Watchtower v1.0.0
// Films & Séries VF/VOSTFR

const watchtowerSources = [{
    "name": "Nova-Stream",
    "langs": ["fr"],
    "ids": { "fr": 703847291 },
    "baseUrl": "https://novastream.top",
    "apiUrl": "https://novastream.top",
    "iconUrl": "https://novastream.top/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "1.1.1",
    ...
    "isAggregator": false,
    "paywall": "free",
    "notes": "Nova-Stream — Films & Séries VF."
}];

const BASE_URL = "https://novastream.top";
...
class DefaultExtension extends MProvider {
    ...
    _parseCards(html) {
        ...
        const re = /<a[^>]+href="(https?:\/\/[^"]+\/(?:film|serie|movie|anime)[^"\/]+[^"]*)"[^>]*>[\s\S]{0,500}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,100})"/gi;
        let m;
        ...
        if (items.length === 0) {
            ...
            while ((m2 = re2.exec(html)) !== null) {
                ...
                if (u in seen || !u.includes(this.baseUrl.replace("https://",""))) continue;
                seen[u] = 1;
    ...
    async getPopular(page) {
        const url = page <= 1 ? this.baseUrl + "/" : `${this.baseUrl}/films/page/${page}/`;
    ...
    async getDetail(url) {
        ...
        const chapters = [];
        const epRe = /<a[^>]+href="([^"]*(?:saison|episode|ep|vf|vostfr)[^"]*)"[^>]*>([\s\S]{0,80}?)<\/a>/gi;
        let em; const eSeen = {};
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://novastream.top",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://novastream.top"
                }
            },
            {
                key: "default_quality",
