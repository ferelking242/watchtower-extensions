const watchtowerSources = [{
      ...
      "langs": ["fr"],
      "ids": { "fr": 471829305 },
      "baseUrl": "https://libertyland.tv",
      "apiUrl": "https://libertyland.tv",
  ...
  const BASE_URL = "https://libertyland.tv";
  class DefaultExtension extends MProvider {
      ...
      _parse(html){
          const list=[],seen={};
          const re=/<a[^>]+href="([^"]+)"[^>]*>[\s\S]{0,400}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,120})"/gi;
          let m;
          while((m=re.exec(html))!==null){
              const url=m[1].startsWith("http")?m[1]:this.baseUrl+(m[1].startsWith("/")?m[1]:"/"+m[1]);
              if(url in seen||url===this.baseUrl||!url.includes("libertyland"))continue;
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://libertyland.tv",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://libertyland.tv"
                }
            },
            {
                key: "default_quality",
