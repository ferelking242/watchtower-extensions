// VoirAnime (voiranime.com) — extension Watchtower v1.0.0
// Anime VF/VOSTFR — site custom
...
const watchtowerSources = [{
    ...
    "isAggregator": false,
    "paywall": "free",
    "notes": "VoirAnime — v6.voiranime.com — VF/VOSTFR. URL miroir modifiable."
}];
...
class DefaultExtension extends MProvider {
    ...
    _parseCards(html) {
        ...
        const re = /<a[^>]+href="((?:https?:\/\/[^"]+)?\/anime\/[^"]+\/)"[^>]*>[\s\S]{0,500}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,100})"/gi;
        let m;
        ...
        if (items.length === 0) {
            const re2 = /<a[^>]+href="([^"]*\/anime\/[^"\/]+[^"]*)"[^>]*>([\s\S]{0,400}?)<\/a>/gi;
            let m2;
    ...
    async getDetail(url) {
        ...
        const chapters = [];
        const epRe = /<a[^>]+href="([^"]*(?:episode|ep-\d|saison|vf|vostfr)[^"]*)"[^>]*>([\s\S]{0,80}?)<\/a>/gi;
        let em; const eSeen = {};
        ...
        return [
            {
                ...
                editTextPreference: {
                    ...
                    value: "https://v6.voiranime.com",
                    dialogTitle: "URL du site",
                    dialogMessage: "URL actuelle : https://v6.voiranime.com"
                }
            },
            {
                key: "default_quality",
