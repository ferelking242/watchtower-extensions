const watchtowerSources = [{
    "name": "Filmzie",
    "lang": "en",
    "baseUrl": "https://filmzie.com",
    "apiUrl": "https://filmzie.com",
    "iconUrl": "https://filmzie.com/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/en/filmzie.js",
    "notes": "Filmzie — free movies streaming",
    "editableBaseUrl": true
}];

const BASE_URL = "https://filmzie.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/movies?page=${page}`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/movies?sort=newest&page=${page}`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async search(query, page, filters) {
        if (!query) return this.getPopular(page);
        const res = await new Client().get(`${this.getBaseUrl()}/search?query=${encodeURIComponent(query)}&page=${page}`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    async getDetail(url) {
    ...
    async getVideoList(url) {
        const fullUrl = url.startsWith("http") ? url : `${this.getBaseUrl()}${url}`;
        return [{ url: fullUrl, quality: "Filmzie", originalUrl: fullUrl }];
    }
