const watchtowerSources = [{
    ...
    "apiUrl": "https://www.popcornflix.com",
    "iconUrl": "https://www.popcornflix.com/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/en/popcornflix.js",
    "notes": "PopcornFlix — free movies & TV with ads",
    "editableBaseUrl": true
}];

const BASE_URL = "https://www.popcornflix.com";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/movies`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: false };
    }
    async getLatestUpdates(page) {
        const res = await new Client().get(`${this.getBaseUrl()}/new`, this.getHeaders());
        return { list: this.parseList(res.body), hasNextPage: false };
    }
    ...
    async getVideoList(url) {
        const fullUrl = url.startsWith("http") ? url : `${this.getBaseUrl()}${url}`;
        return [{ url: fullUrl, quality: "PopcornFlix", originalUrl: fullUrl }];
    }
