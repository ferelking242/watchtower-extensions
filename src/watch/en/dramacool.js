const watchtowerSources = [{
    "name": "DramaCool",
    "lang": "en",
    "baseUrl": "https://dramacool.ps",
    "apiUrl": "https://dramacool.ps",
    "iconUrl": "https://dramacool.ps/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/en/dramacool.js",
    "notes": "DramaCool — Korean & Asian drama streaming",
    "editableBaseUrl": true
}];

const BASE_URL = "https://dramacool.ps";
...
class DefaultExtension extends MProvider {
    ...
    async getDetail(url) {
        ...
        const html = res.body;
        const nameM = html.match(/<h1[^>]*>([^<]+)<\/h1>/i) || html.match(/<h2[^>]*class="[^"]*title[^"]*"[^>]*>([^<]+)/i);
        const name = nameM ? nameM[1].trim() : "";
        const descM = html.match(/<p[^>]*class="[^"]*description[^"]*"[^>]*>([\s\S]*?)<\/p>/i) || html.match(/<div[^>]*class="[^"]*summary[^"]*"[^>]*>([\s\S]*?)<\/div>/i);
        const description = descM ? descM[1].replace(/<[^>]+>/g, "").trim() : "";
        const imgM = html.match(/<img[^>]+class="[^"]*(?:poster|cover|drama-img)[^"]*"[^>]+src="([^"]+)"/i);
        const imageUrl = imgM ? imgM[1] : "";
        const episodes = [];
        const epRe = /<a[^>]+href="(\/[^"]*(?:episode|ep)[^"]*)"[^>]*>\s*([^<]*(?:Episode|Ep\.?|Eps?)\s*\d+[^<]*)<\/a>/gi;
        let em;
        while ((em = epRe.exec(html)) !== null) {
            episodes.push({ name: em[2].trim(), url: em[1], dateUpload: "" });
        }
        if (episodes.length === 0) {
            const epRe2 = /href="(\/[^"]*ep[^"]*)"[^>]*>\s*(?:Episode\s*)?(\d+)/gi;
            while ((em = epRe2.exec(html)) !== null) { episodes.push({ name: `Episode ${em[2]}`, url: em[1], dateUpload: "" }); }
        }
