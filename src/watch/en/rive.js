const watchtowerSources = [{
    ...
    "apiUrl": "https://rivestream.live",
    "iconUrl": "https://rivestream.live/favicon.ico",
    "typeSource": "single",
    "itemType": 1,
    "version": "0.1.0",
    "pkgPath": "watch/en/rive.js",
    "notes": "Rive — movies, TV, anime stream aggregator",
    "editableBaseUrl": true
}];

const BASE_URL = "https://rivestream.live";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        ...
        return { list: this.parseList(res.body), hasNextPage: res.body.includes(`page=${page + 1}`) };
    }
    ...
    async getDetail(url) {
        ...
        const descM = html.match(/<p[^>]*class="[^"]*desc[^"]*"[^>]*>([\s\S]*?)<\/p>/i) || html.match(/<p[^>]*>([\s\S]*?)<\/p>/i);
        const description = descM ? descM[1].replace(/<[^>]+>/g, "").trim() : "";
        const imgM = html.match(/<img[^>]+src="([^"]+)"/i);
        const imageUrl = imgM ? imgM[1] : "";
        const episodes = [];
        const epRe = /<a[^>]+href="(\/[^"]*(?:season|episode|ep|server)[^"]*)"[^>]*>\s*([^<]+)<\/a>/gi;
        let em;
        while ((em = epRe.exec(html)) !== null) {
            episodes.push({ name: em[2].trim(), url: em[1], dateUpload: "" });
        }
        if (episodes.length === 0) {
            const vidRe2 = /href="(\/[^"]+)"[\s\S]*?>([^<]*(?:Season|Episode|Ep)[^<]*)<\/a>/gi;
            while ((em = vidRe2.exec(html)) !== null) {
                episodes.push({ name: em[2].trim(), url: em[1], dateUpload: "" });
            }
        }
        return { name, description, imageUrl, genres: [], status: 0, chapters: episodes.length > 0 ? episodes : [{ name, url, dateUpload: "" }] };
    }
