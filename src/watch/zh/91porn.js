...
class DefaultExtension extends MProvider {
  ...
  _parse(html, base) {
    ...
    const items = [];
    const cards = doc.select(".well, .video-item, .thumb, li[class*='video'], .videoWrapper");
    for (const card of cards) {
      ...
      const title = img?.attr("alt") || a.attr("title") || card.selectFirst(".title, h3, h2, span.title")?.text || "";
