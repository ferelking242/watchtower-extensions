const watchtowerSources = [{
  "name": "BestJavPorn",
  "lang": "ja",
  "baseUrl": "https://bestjavporn.com",
  "apiUrl": "",
  "iconUrl": "https://bestjavporn.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "bestjavporn/ja/ja.bestjavporn.js",
  "notes": "Best JAV Porn — free Japanese adult videos",
  "isNsfw": true
}];
...
class DefaultExtension extends MProvider {
  ...
  async getLatestUpdates(page) {
    const url = `${this.source.baseUrl}/new/?page=${page}`;
    const res = await new Client().get(url, { headers: this.getHeaders(url) });
    return this._parse(res.body, this.source.baseUrl);
  }
  ...
  _parse(html, base) {
    ...
    const items = [];
    const cards = doc.select(".item, article, .post, .video-item, [class*='thumb'], [class*='movie']");
    for (const card of cards) {
      const a = card.selectFirst("a[href*='bestjavporn'], h2 a, h3 a, a");
      if (!a) continue;
