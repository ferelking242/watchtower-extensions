const watchtowerSources = [{
  "name": "porndoe",
  "lang": "en",
  "baseUrl": "https://www.porndoe.com",
  "apiUrl": "",
  "iconUrl": "https://www.porndoe.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/porndoe.js",
  "notes": "Adult content (18+) — porndoe free tube",
  "isNsfw": true
}];

class DefaultExtension extends MProvider {
  getHeaders(url) {
    return { "Referer": "https://www.porndoe.com/", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36", "Accept-Language": "en-US,en;q=0.9" };
  }
  async getPopular(page) {
    const res = await new Client().get("https://www.porndoe.com/most-popular/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  get supportsLatest() { return true; }
  async getLatestUpdates(page) {
    const res = await new Client().get("https://www.porndoe.com/latest-updates/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  async search(query, page, filters) {
    const q = encodeURIComponent(query.trim());
    const res = await new Client().get("https://www.porndoe.com/search/" + q + "/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  _parse(html, page) {
    ...
    for (const card of cards) {
      ...
      const link = href.startsWith("http") ? href : "https://www.porndoe.com" + href;
