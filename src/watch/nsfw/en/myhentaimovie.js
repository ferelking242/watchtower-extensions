const watchtowerSources = [{
  "name": "MyHentaiMovie",
  "lang": "en",
  "baseUrl": "https://myhentaimovie.com",
  "apiUrl": "",
  "iconUrl": "https://myhentaimovie.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/myhentaimovie.js",
  "notes": "MyHentaiMovie streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://myhentaimovie.com";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.anime-item,.movie-item")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.movie-title")?.text?.trim() || a.attr("title") || "Hentai";
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.movie-title")?.text?.trim() || "Hentai";
