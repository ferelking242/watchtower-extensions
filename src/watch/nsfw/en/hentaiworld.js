const watchtowerSources = [{
  ...
  "apiUrl": "",
  "iconUrl": "https://hentaiworld.tv/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/hentaiworld.js",
  "notes": "HentaiWorld streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://hentaiworld.tv";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    for (const card of doc.select("article,.video-item,.item,.anime-item,.film-item")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.anime-name,.film-name")?.text?.trim() || a.attr("title") || "Hentai";
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.anime-name,.film-name")?.text?.trim() || "Hentai";
