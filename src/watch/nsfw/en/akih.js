const watchtowerSources = [{
  ...
  "apiUrl": "",
  "iconUrl": "https://aki-h.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/akih.js",
  "notes": "Aki-H hentai streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://aki-h.com";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.anime-item,.film-item,.entry")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.film-name,.entry-title")?.text?.trim() || a.attr("title") || "Hentai";
      items.push({ name, imageUrl: thumb, link });
    }
    return { list: items, hasNextPage: !!doc.selectFirst("a.next,[rel=next],.pagination-next,.nav-next") };
  }
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.film-name,.entry-title")?.text?.trim() || "Hentai";
    const imageUrl = doc.selectFirst('meta[property="og:image"]')?.attr("content") || "";
    const description = doc.selectFirst(".description,.synopsis,.entry-content p")?.text?.trim() || "";
