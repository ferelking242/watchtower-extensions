const watchtowerSources = [{
  "name": "OceanVeil",
  "lang": "en",
  "baseUrl": "https://oceanveil.org",
  "apiUrl": "",
  "iconUrl": "https://oceanveil.org/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/oceanveil.js",
  "notes": "OceanVeil hentai streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://oceanveil.org";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.anime-item,.film-item,.episode-item")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.film-name")?.text?.trim() || a.attr("title") || "Hentai";
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.film-name,.anime-title")?.text?.trim() || "Hentai";
