const watchtowerSources = [{
  "name": "girlstop",
  "lang": "en",
  "baseUrl": "https://girlstop.info",
  "apiUrl": "",
  "iconUrl": "https://girlstop.info/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/girlstop.js",
  "notes": "Adult content (18+) — GirlsTop best model rankings",
  "isNsfw": true
}];

class DefaultExtension extends MProvider {
  getHeaders(url) {
    return { "Referer": "https://girlstop.info/", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36", "Accept-Language": "en-US,en;q=0.9" };
  }
  async getPopular(page) {
    const res = await new Client().get("https://girlstop.info/popular?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  get supportsLatest() { return true; }
  async getLatestUpdates(page) {
    const res = await new Client().get("https://girlstop.info/latest?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  async search(query, page, filters) {
    const q = encodeURIComponent(query.trim());
    const res = await new Client().get("https://girlstop.info/search/" + q + "?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  _parse(html, page) {
    ...
    for (const card of cards) {
      ...
      const link = href.startsWith("http") ? href : "https://girlstop.info" + href;
