const watchtowerSources = [{
  "name": "asianude4u",
  "lang": "en",
  "baseUrl": "https://asianude4u.net",
  "apiUrl": "",
  "iconUrl": "https://asianude4u.net/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/asianude4u.js",
  "notes": "Adult content (18+) — AsiaNude4u Asian nude photos",
  "isNsfw": true
}];

class DefaultExtension extends MProvider {
  getHeaders(url) {
    return { "Referer": "https://asianude4u.net/", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36", "Accept-Language": "en-US,en;q=0.9" };
  }
  async getPopular(page) {
    const res = await new Client().get("https://asianude4u.net/popular?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  get supportsLatest() { return true; }
  async getLatestUpdates(page) {
    const res = await new Client().get("https://asianude4u.net/latest?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  async search(query, page, filters) {
    const q = encodeURIComponent(query.trim());
    const res = await new Client().get("https://asianude4u.net/search/" + q + "?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  _parse(html, page) {
    ...
    for (const card of cards) {
      ...
      const link = href.startsWith("http") ? href : "https://asianude4u.net" + href;
