const watchtowerSources = [{
  "name": "fuxxx",
  "lang": "en",
  "baseUrl": "https://fuxxx.com",
  "apiUrl": "",
  "iconUrl": "https://fuxxx.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/fuxxx.js",
  "notes": "Adult content (18+) — fuxxx free tube",
  "isNsfw": true
}];

class DefaultExtension extends MProvider {
  getHeaders(url) {
    return { "Referer": "https://fuxxx.com/", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36", "Accept-Language": "en-US,en;q=0.9" };
  }
  async getPopular(page) {
    const res = await new Client().get("https://fuxxx.com/most-popular/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  get supportsLatest() { return true; }
  async getLatestUpdates(page) {
    const res = await new Client().get("https://fuxxx.com/latest-updates/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  async search(query, page, filters) {
    const q = encodeURIComponent(query.trim());
    const res = await new Client().get("https://fuxxx.com/search/" + q + "/?page=" + page, this.getHeaders());
    return this._parse(res.body, page);
  }
  _parse(html, page) {
    ...
    for (const card of cards) {
      ...
      const link = href.startsWith("http") ? href : "https://fuxxx.com" + href;
