const watchtowerSources = [{
  "name": "Hentaisea",
  "lang": "en",
  "baseUrl": "https://hentaisea.com",
  "apiUrl": "",
  "iconUrl": "https://hentaisea.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/hentaisea.js",
  "notes": "Hentaisea streaming (18+)",
  "isNsfw": true
}];
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.anime-item,.episode-item")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.anime-title")?.text?.trim() || a.attr("title") || "Hentai";
  ...
  async getPopular(page) {
    const res = await new Client().get(`${BASE}/?page=${page}&sort=views`, this.getHeaders());
  ...
  async getDetail(url) {
    ...
    const episodes = [];
    for (const ep of doc.select(".episode-list a,.ep-item a,a[href*=episode]")) {
