const watchtowerSources = [{
  "name": "HentaiStream",
  "lang": "en",
  "baseUrl": "https://hentaistream.com",
  "apiUrl": "",
  "iconUrl": "https://hentaistream.com/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/hentaistream.js",
  "notes": "HentaiStream (18+)",
  "isNsfw": true
}];

const BASE = "https://hentaistream.com";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    for (const card of doc.select("article,.video-item,.item,.anime-item")) {
      ...
      const name = card.selectFirst("h1,h2,h3,.title,.anime-title")?.text?.trim() || a.attr("title") || "Hentai";
  ...
  async getDetail(url) {
    ...
    const episodes = [];
    for (const ep of doc.select(".episode-list a,a[href*=episode],a[href*=-episode-]")) {
