const watchtowerSources = [{
  "name": "WatchHentai",
  "lang": "en",
  "baseUrl": "https://watch-hentai.net",
  "apiUrl": "",
  "iconUrl": "https://watch-hentai.net/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/watchhentai.js",
  "notes": "WatchHentai streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://watch-hentai.net";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.anime-item,.item")) {
      ...
      const name = card.selectFirst("h1,h2,h3,.title,.anime-title")?.text?.trim() ||
                   a.attr("title") || "Hentai";
  ...
  async getPopular(page) {
    const res = await new Client().get(`${BASE}/hentai-videos/popular/?page=${page}`, this.getHeaders());
    return this._parse(res.body);
  }

  async getLatestUpdates(page) {
    const res = await new Client().get(`${BASE}/hentai-videos/?page=${page}`, this.getHeaders());
    return this._parse(res.body);
  }
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.anime-title")?.text?.trim() || "Hentai";
    const imageUrl = doc.selectFirst('meta[property="og:image"]')?.attr("content") ||
                     doc.selectFirst(".anime-image img,.poster img")?.attr("src") || "";
    const description = doc.selectFirst(".description,.summary,.synopsis,.anime-desc")?.text?.trim() || "";
    const genre = doc.select("a[rel=tag],.tags a,.genres a").map(el => ({ name: el.text.trim() }));
    // Get episodes
    const episodes = [];
    for (const ep of doc.select("a[href*=episode], a[href*=ep-], .episode-item a")) {
      const epLink = ep.attr("href") || "";
      if (!epLink || epLink === "#") continue;
      const epUrl = epLink.startsWith("http") ? epLink : BASE + epLink;
      episodes.push({ name: ep.text?.trim() || ep.attr("title") || "Episode", url: epUrl });
    }
