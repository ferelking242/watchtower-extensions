const watchtowerSources = [{
  ...
  "apiUrl": "",
  "iconUrl": "https://oppai.stream/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/oppaistream.js",
  "notes": "oppai.stream hentai streaming (18+)",
  "isNsfw": true
}];

const BASE = "https://oppai.stream";
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.episode-item,.film-item")) {
  ...
  async getPopular(page) {
    const res = await new Client().get(`${BASE}/?page=${page}&sort=trending`, this.getHeaders());
