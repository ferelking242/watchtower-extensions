const watchtowerSources = [{
  ...
  "apiUrl": "",
  "iconUrl": "https://hentaimama.io/favicon.ico",
  "typeSource": "single",
  "itemType": 1,
  "version": "1.0.0",
  "pkgPath": "watch/nsfw/en/hentaimama.js",
  "notes": "HentaiMama streaming (18+)",
  "isNsfw": true
}];
...
class DefaultExtension extends MProvider {
  ...
  _parse(html) {
    ...
    const items = [];
    const seen = new Set();
    for (const card of doc.select("article,.video-item,.item,.anime-item,.film-item")) {
      ...
      const name = card.selectFirst("h2,h3,.title,.anime-title,.film-name")?.text?.trim() ||
                   a.attr("title") || "Hentai";
  ...
  async getPopular(page) {
    const res = await new Client().get(`${BASE}/episodes/?page=${page}&order=popular`, this.getHeaders());
    return this._parse(res.body);
  }

  async getLatestUpdates(page) {
    const res = await new Client().get(`${BASE}/episodes/?page=${page}`, this.getHeaders());
    return this._parse(res.body);
  }
  ...
  async getDetail(url) {
    ...
    const name = doc.selectFirst("h1,.anime-title,.film-name")?.text?.trim() || "Hentai";
    const imageUrl = doc.selectFirst('meta[property="og:image"]')?.attr("content") ||
                     doc.selectFirst(".cover img,.poster img,.film-poster img")?.attr("src") || "";
    ...
    const episodes = [];
    for (const ep of doc.select(".ep-item a,.episode-list a,a[href*=episode],a[href*=-ep-]")) {
