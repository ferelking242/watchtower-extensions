const watchtowerSources = [{
    "name": "Japscan",
    "langs": ["fr"],
    "ids": { "fr": 738291504 },
    "baseUrl": "https://www.japscan.lol",
    "apiUrl": "https://www.japscan.lol",
    "iconUrl": "https://www.japscan.lol/favicon.ico",
    "typeSource": "single",
    "itemType": 0,
    "isManga": true,
    "version": "0.1.2",
    "pkgPath": "manga/fr/japscan.js",
    "editableBaseUrl": true,
    "hasCloudflare": false,
    "requiresAccount": false,
    "hasDRM": false,
    "paywall": "free",
    "notes": "Scans manga FR — Japscan (images canvas obfusquées côté client)"
}];
const BASE_URL = "https://www.japscan.lol";
class DefaultExtension extends MProvider {
    constructor(){super();}
    get baseUrl() { return new SharedPreferences().get("base_url") || BASE_URL; }

    getSourcePreferences() {
        return [{
            key: "base_url",
            editTextPreference: {
                title: "URL du site",
                summary: "Adresse du site Japscan. Modifiez-la uniquement si le domaine a changé (migration ou miroir).",
                value: BASE_URL,
                dialogTitle: "URL du site",
                dialogMessage: "URL actuelle : " + BASE_URL
            }
        }];
    }
    _hdrs(ref){return{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124 Safari/537.36","Referer":ref||this.baseUrl+"/","Accept-Language":"fr-FR,fr;q=0.9"};}
    _dec(s){return String(s||"").replace(/&amp;/g,"&").replace(/&quot;/g,'"').replace(/&#039;/g,"'").replace(/&eacute;/g,"é").replace(/&agrave;/g,"à").replace(/&ccedil;/g,"ç").replace(/<[^>]+>/g,"").replace(/\s+/g," ").trim();}
    _parse(html){
        const list=[],seen={};
        const re=/<a[^>]+href="([^"]+)"[^>]*>[\s\S]{0,300}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]*(?:alt|title)="([^"]{2,120})"/gi;
        let m;
        while((m=re.exec(html))!==null){
            const url=m[1].startsWith("http")?m[1]:this.baseUrl+(m[1].startsWith("/")?m[1]:"/"+m[1]);
            if(url in seen||url===this.baseUrl||url.endsWith("/")||url.length<25)continue;
            seen[url]=1;list.push({link:url,imageUrl:m[2],name:this._dec(m[3].trim())});
        }
        return list;
    }
    async getPopular(page){const r=await new Client().get(`${this.baseUrl}/mangas/${page}/`,this._hdrs());return{list:this._parse(r.body),hasNextPage:page<50};}
    async getLatestUpdates(page){const r=await new Client().get(this.baseUrl+"/",this._hdrs());return{list:this._parse(r.body),hasNextPage:false};}
    async search(query,page,f){const r=await new Client().get(`${this.baseUrl}/recherche?q=${encodeURIComponent(query)}`,this._hdrs());return{list:this._parse(r.body),hasNextPage:false};}
    async getDetail(url){
        const r=await new Client().get(url,this._hdrs(url));const html=r.body;
        const nameM=html.match(/<h1[^>]*>([\s\S]*?)<\/h1>/);
        const name=nameM?this._dec(nameM[1]):"";
        const imgM=html.match(/<meta[^>]+property="og:image"[^>]+content="([^"]+)"/i)
                 ||html.match(/class="[^"]*cover[^"]*"[\s\S]{0,200}?<img[^>]+src="([^"]+)"/i);
        const descM=html.match(/<meta[^>]+name="description"[^>]+content="([^"]+)"/i);
        const chapters=[],seen={};
        // Chapitres Japscan : /lecture-en-ligne/{manga}/{chap}/
        const chRe=/<a[^>]+href="(\/lecture-en-ligne\/[^"]+)"[^>]*>([\s\S]{1,150}?)<\/a>/gi;let m;
        while((m=chRe.exec(html))!==null){
            const cu=this.baseUrl+m[1];
            if(cu in seen)continue;seen[cu]=1;
            const label=this._dec(m[2])||m[1].split("/").filter(Boolean).pop()||"Chapitre";
            chapters.push({name:label,url:cu});
        }
        return{name,imageUrl:imgM?imgM[1]:"",description:descM?this._dec(descM[1]):"",chapters:chapters.length>0?chapters:[{name:name||"Lire",url}]};
    }
    async getPageList(url){
        const r=await new Client().get(url,this._hdrs(url));const html=r.body;
        const pages=[],seen=new Set();
        // Japscan: URLs d'image dans les variables JS
        const imgVarRe=/["']((https?:)?\/\/[^"']+\.(?:jpg|jpeg|png|webp)[^"']{0,80})["']/gi;let m;
        while((m=imgVarRe.exec(html))!==null){
            let u=m[1];if(!u.startsWith("http"))u="https:"+u;
            if(!seen.has(u)&&!u.includes("logo")&&!u.includes("banner")&&!u.includes("favicon")){seen.add(u);pages.push({url:u,headers:this._hdrs(url)});}
        }
        if(pages.length===0)pages.push({url,headers:this._hdrs()});
        return pages;
    }
    getForYou(page){return this.getPopular(page);}
    getComments(url,page){return Promise.resolve([]);}
}