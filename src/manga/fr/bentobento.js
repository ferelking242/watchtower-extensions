...
const BASE_URL = "https://bentobento.fr";
class DefaultExtension extends MProvider {
    ...
    getSourcePreferences() {
        return [{
            ...
            editTextPreference: {
                title: "URL du site",
                summary: "Adresse du site BentoBento. Modifiez-la uniquement si le domaine a changé (migration ou miroir).",
                value: BASE_URL,
    ...
    async getDetail(url){
        ...
        if(chapters.length===0){const idM=html.match(/manga_id['"]\s*:\s*['"]?(\d+)/i)||html.match(/data-id="(\d+)"/);
            if(idM){try{const aj=await new Client().post(this.baseUrl+"/wp-admin/admin-ajax.php",this._hdrs(url),"action=manga_get_chapters&manga="+idM[1]);
                ...
                while((am=aRe.exec(aj.body))!==null){const cu=am[1].startsWith("http")?am[1]:this.baseUrl+am[1];if(cu in seen||cu===url)continue;seen[cu]=1;chapters.push({name:this._dec(am[2]),url:cu});}}catch(_){}}}
    ...
    async getPageList(url){
        ...
        if(jsArr){try{const arr=JSON.parse(jsArr[1].replace(/'/g,'"'));for(const i of arr){const u=typeof i==="string"?i:(i.url||i.src||"");if(u&&!seen.has(u)){seen.add(u);pages.push({url:u,headers:this._hdrs(url)});}}}catch(_){}}
        if(pages.length===0){const imgRe=/<img[^>]+(?:data-src|src)="([^"]+\.(?:jpg|jpeg|png|webp)[^"]{0,80})"/gi;let m;while((m=imgRe.exec(html))!==null){const u=m[1].startsWith("http")?m[1]:this.baseUrl+m[1];if(!seen.has(u)&&!u.includes("logo")&&!u.includes("banner")&&!u.includes("favicon")){seen.add(u);pages.push({url:u,headers:this._hdrs(url)});}}}
        return pages;
    }
