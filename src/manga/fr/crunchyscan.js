const watchtowerSources = [{
    ...
    "apiUrl": "https://crunchyscan.fr",
    "iconUrl": "https://crunchyscan.fr/favicon.ico",
    "typeSource": "single",
    "itemType": 0,
    "isManga": true,
    "version": "0.1.2",
    "pkgPath": "manga/fr/crunchyscan.js",
...
const BASE_URL = "https://crunchyscan.fr";
class DefaultExtension extends MProvider {
    ...
    getSourcePreferences() {
        return [{
            ...
            editTextPreference: {
                title: "URL du site",
                summary: "Adresse du site CrunchyScan. Modifiez-la uniquement si le domaine a changé (migration ou miroir).",
                value: BASE_URL,
    ...
    _parse(html){
        const list=[],seen={};
        const re=/<a[^>]+href="([^"]+)"[^>]*>[\s\S]{0,300}?<img[^>]+(?:src|data-src)="([^"]+)"[^>]+alt="([^"]{2,120})"/gi;
        let m;
    ...
    async getPopular(page){const r=await new Client().get(this.baseUrl+"/manga/?m_orderby=views&page="+page,this._hdrs());return{list:this._parse(r.body),hasNextPage:page<20};}
    async getLatestUpdates(page){const r=await new Client().get(this.baseUrl+"/manga/?m_orderby=latest&page="+page,this._hdrs());return{list:this._parse(r.body),hasNextPage:page<20};}
    ...
    async getDetail(url){
        ...
        if(chapters.length===0){
            const idM=html.match(/manga_id['"]\s*:\s*['"]?(\d+)/i)||html.match(/data-id="(\d+)"/);
