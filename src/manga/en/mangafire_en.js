...
class DefaultExtension extends MProvider {
    ...
    async getPopular(p) { const r=await new Client().get(`${BASE_URL}/filter?page=${p}&sort=most_watched`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:r.body.includes('page=')}; }
    async getLatestUpdates(p) { const r=await new Client().get(`${BASE_URL}/home?page=${p}`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:r.body.includes('page=')}; }
    async search(q,p) { if(!q)return this.getPopular(p);const r=await new Client().get(`${BASE_URL}/filter?q=${encodeURIComponent(q)}&page=${p}`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:false}; }
    async getDetail(url) { const u=url.startsWith("http")?url:`${BASE_URL}${url}`;const r=await new Client().get(u,this.getHeaders());const h=r.body;const nm=h.match(/<h1[^>]*>([^<]+)<\/h1>/);const n=nm?nm[1].trim():"";const dm=h.match(/<meta[^>]+name="description"[^>]+content="([^"]+)"/);const d=dm?dm[1]:"";const im=h.match(/<img[^>]+src="([^"]+)"[^>]*(?:poster|cover)/i);const img=im?im[1]:"";const genre=[];const gr=/genre[^>]*>([^<]+)/gi;let gm;while((gm=gr.exec(h))!==null){const g=gm[1].trim();if(g&&g.length<25)genre.push(g);}const ch=[];const cr=/href="([^"]+)"[\s\S]*?chapter[\s\S]*?([\d.]+)/gi;let cm;while((cm=cr.exec(h))!==null){ch.push({name:`Ch. ${cm[2]}`,url:cm[1]});}return{name:n,description:d,imageUrl:img,genre,status:0,chapters:ch}; }
