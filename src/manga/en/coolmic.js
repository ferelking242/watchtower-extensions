...
class DefaultExtension extends MProvider {
    ...
    async getPopular(p) { const r=await new Client().get(`${BASE_URL}/series?page=${p}&sort=POPULAR`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:r.body.includes('page=')}; }
    async getLatestUpdates(p) { const r=await new Client().get(`${BASE_URL}/series?page=${p}&sort=LATEST`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:r.body.includes('page=')}; }
    async search(q,p) { if(!q)return this.getPopular(p);const r=await new Client().get(`${BASE_URL}/search?q=${encodeURIComponent(q)}`,this.getHeaders());return{list:this.mangaListParse(r.body),hasNextPage:false}; }
