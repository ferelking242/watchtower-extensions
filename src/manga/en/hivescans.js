const watchtowerSources = [{
    ...
    "itemType": 0,
    "version": "1.1.0",
    "pkgPath": "manga/src/en/hivescans.js"
}];
...
class DefaultExtension extends MProvider {
    ...
    async getDetail(url) {
        ...
        const imgM = html.match(/<img[^>]+src="([^"]+)"[^>]*alt="[^"]*"/);
        ...
        while ((gm = genreRe.exec(html)) !== null) {
            const g = gm[1].trim();
            if (g && g.length < 25 && !g.toLowerCase().includes("genre")) genre.push(g);
        }
        ...
        const chapters = [];
        const chRe = /href="([^"]*chapter[^"]*)"[^>]*>([^<]*(?:chapter|ch\.?)\s*[\d.]+[^<]*)/gi;
        let cm;
    ...
    async getPageList(url) {
        ...
        const html = res.body;
        const pages = [];
        const re = /src="([^"]+)"[^>]*(?:chapter-img|reading-content|wp-manga-chapter-img)/gi;
        let m;
        while ((m = re.exec(html)) !== null) {
            if (!m[1].includes("logo") && !m[1].includes("icon")) pages.push(m[1]);
        }
        if (pages.length === 0) {
