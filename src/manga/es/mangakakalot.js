const watchtowerSources = [{
    ...
    "apiUrl": "",
    "iconUrl": "https://mangakakalot.com/favicon.ico",
    "typeSource": "single",
    "itemType": 0,
    "version": "1.1.0",
    "pkgPath": "manga/src/es/mangakakalot.js"
}];
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) { const res = await new Client().get(`${BASE_URL}/manga_list?type=topview&category=all&page=${page}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: res.body.includes('page=') }; }
    async getLatestUpdates(page) { const res = await new Client().get(`${BASE_URL}/manga_list?type=latest&category=all&page=${page}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: res.body.includes('page=') }; }
    async search(query, page, filters) { if (!query) return this.getPopular(page); const res = await new Client().get(`${BASE_URL}/search/story/${encodeURIComponent(query)}?page=${page}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: false }; }
