const watchtowerSources = [{
    "name": "MangaSee",
    "lang": "es",
    "baseUrl": "https://mangaseeonline.us",
    "apiUrl": "",
    "iconUrl": "https://mangaseeonline.us/favicon.ico",
    "typeSource": "single",
    "itemType": 0,
    "version": "1.1.0",
    "pkgPath": "manga/src/es/mangasee.js"
}];

const BASE_URL = "https://mangaseeonline.us";
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) { const res = await new Client().get(`${BASE_URL}/directory?page=${page}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: res.body.includes('page=') }; }
    async getLatestUpdates(page) { const res = await new Client().get(`${BASE_URL}/latest?page=${page}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: res.body.includes('page=') }; }
    async search(query, page, filters) { if (!query) return this.getPopular(page); const res = await new Client().get(`${BASE_URL}/search?keyword=${encodeURIComponent(query)}`, this.getHeaders()); return { list: this.mangaListParse(res.body), hasNextPage: false }; }
    async getDetail(url) { const fullUrl = url.startsWith("http") ? url : `${BASE_URL}${url}`; const res = await new Client().get(fullUrl, this.getHeaders()); const html = res.body; const nameM = html.match(/<h1[^>]*>([^<]+)<\/h1>/); const name = nameM ? nameM[1].trim() : ""; const descM = html.match(/<meta[^>]+name="description"[^>]+content="([^"]+)"/); const description = descM ? descM[1] : ""; const imgM = html.match(/<img[^>]+src="([^"]+)"[^>]*(?:cover|manga)/i); const imageUrl = imgM ? imgM[1] : ""; const genre = []; const chapters = []; const chRe = /href="([^"]+)"[\s\S]*?(?:chapter|ch\.?)\s*([\d.]+)/gi; let cm; while ((cm = chRe.exec(html)) !== null) { chapters.push({ name: `Ch. ${cm[2]}`, url: cm[1] }); } return { name, description, imageUrl, genre, status: 0, chapters }; }
        ...
        return [
            ...
            },
            {
                ...
                listPreference: {
                    ...
                    valueIndex: 0,
                    entries: ["Espagnol (recommandé)", "Anglais", "Automatique"],
                    entryValues: ["es", "en", "auto"]
                }
            },
            {
                key: "chapter_order",
