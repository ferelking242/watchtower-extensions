    ...
    try {
      if (kIsWeb) {
        ...
        if (status.isPermanentlyDenied) {
          ...
          granted = result.isGranted;
          // iOS: after the system dialog, re-check the real status
          // because the result is sometimes not yet settled.
          if (!granted && !kIsWeb && Platform.isIOS) {
            await Future.delayed(const Duration(milliseconds: 600));
            final rechk = await Permission.notification.status;
            granted = rechk.isGranted;
          }
        }
      }
    } catch (_) {
    ...
    return Row(
      children: [
        Container(
          ...
          height: 46,
          decoration: BoxDecoration(
            color: Colors.white.withValues(alpha: dimmed ? 0.09 : 0.08),
            borderRadius: BorderRadius.circular(12),
          ),
          child: Icon(icon,
              color: Colors.white.withValues(alpha: dimmed ? 0.62 : 0.7), size: 22),
        ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(title,
                  style: TextStyle(
                      color: Colors.white.withValues(alpha: dimmed ? 0.78 : 1.0),
                      fontSize: 15,
                      fontWeight: FontWeight.w600)),
              const SizedBox(height: 2),
              Text(subtitle,
                  style: TextStyle(
                      color: Colors.white.withValues(alpha: dimmed ? 0.50 : 0.42),
                      fontSize: 12,
                      height: 1.4)),
            ],
          ),
        ...
        if (granted)
          ...
          InkWell(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color:
                    Colors.white.withValues(alpha: busy ? 0.04 : (dimmed ? 0.12 : 0.10)),
                borderRadius: BorderRadius.circular(10),
                border: Border.all(
                    color: Colors.white.withValues(alpha: dimmed ? 0.18 : 0.15),
                    width: 1),
              ),
              child: busy
                  ? const SizedBox(
                  ...
                  : Text('Autoriser',
                      style: TextStyle(
                          color: Colors.white.withValues(alpha: dimmed ? 0.72 : 1.0),
                          fontSize: 13,
                          fontWeight: FontWeight.w600)),
            ),
          ),
