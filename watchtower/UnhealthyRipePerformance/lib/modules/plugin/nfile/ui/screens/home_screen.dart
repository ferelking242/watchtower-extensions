...
class HomeScreen extends StatefulWidget {
  ...
  /// When [embedded] is true (launched from Watchtower via go_router) the
  /// nfile bottom navigation bar is hidden — Watchtower's dock takes over.
  final bool embedded;
  const HomeScreen({super.key, required this.toggleTheme, this.embedded = false});
...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver, SingleTickerProviderStateMixin {
  ...
  void _navigateBackToWatchtower(BuildContext context) {
    try {
      GoRouter.of(context).go('/plugins');
    } catch (_) {
  ...
  void _showExitConfirmationDialog(BuildContext context) {
    // When embedded in Watchtower, back always returns to Watchtower.
    if (widget.embedded) {
      _navigateBackToWatchtower(context);
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        bottomNavigationBar: (!widget.embedded && provider.showHomeBrowseNav)
            ? NavigationBar(
