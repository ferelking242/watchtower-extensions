...
// Channel shared with the Android ShortcutManager handler in MainActivity.kt.
...
class _PluginCard extends StatelessWidget {
  ...
  static Future<void> _pinShortcut(BuildContext context, String id, String label) async {
    try {
      final supported = await _shortcutChannel.invokeMethod<bool>('isSupported') ?? false;
      if (!supported) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Raccourcis non supportés par ce launcher'),
      ...
      await _shortcutChannel.invokeMethod('pinShortcut', {'id': id, 'label': label});
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Raccourci « $label » ajouté à l\'accueil'),
            behavior: SnackBarBehavior.floating,
          ),
        );
      }
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erreur raccourci : $e'),
            behavior: SnackBarBehavior.floating,
          ),
        );
      }
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () async {
                    Navigator.of(sheetCtx).pop();
                    await _pinShortcut(context, id, label);
                  },
                ),
